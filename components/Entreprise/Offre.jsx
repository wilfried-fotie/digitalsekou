...
import { AlignMiddle, ArrowLeft, ArrowRight, Building, Calendar2DateFill, CalendarX, DisplayFill, EyeFill, FileImageFill, ImageFill, Link as Lk, PaintBucket, PencilFill, PencilSquare, PenFill, Person, PhoneFill, PhoneVibrateFill, PinFill, TelephoneFill, TrashFill, TrophyFill } from 'react-bootstrap-icons'
...
function Offre() {
    ...
    const [state, setState] = React.useState({ logo:  site.logo ?  "/" + site.name + "-entreprise-" + site.logo : "" ,url: "", outro: "", objet: "", expiration: "",tel: site.tel || "" })
            ...
            <div className={styles.anim}>
                ...
                <div className={data.content ? styles.aff : styles.offer}>
                    ...
                    <div>
                        ...
                        <p className="pad">
                            <b>Objet:</b>   {data.objet ? data.objet : "Objectif de l'offre: Exp Recrutement de 2 devs"}
                        </p>

                        <center className="fine">Expire Le:  {" " + new Date(data.expiration).toDateString() || " date d'expiration"}</center>
                        <div className="pad">
                            <div className={styles.view}><Link href={data.url}><a className="btnSecondary dfss"><EyeFill size={20} color="#4a00b4"  /> voir l'offre </a></Link></div>
 </div>
                        <div className="pad">
                            {data.outro && <Markup content={draftToHtml(data.outro)} />}
                            {data.content && <Markup content={ data.content.substr(0, 400) + "..."} /> }
                        </div>
                    </div>
                </div>
...
export function Creation({ onHandleImageStateChange, onHandleTextStateChange, data, onHandleEditor }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (data.tel && data.logo && data.objet && data.url && data.expiration && data.outro) {
            
setLoader(true)
                if (data.logo == "/" + site.name + "-entreprise-" + site.logo) {
                    axios.post("/add-offer-entreprise/" + entreprise.id,{
                        ...data, logo: "/" + site.name + "-entreprise-" + site.logo,outro: draftToHtml(data.outro)
                    }).then(res => {
                ...
                setLoader(true)
                formData1.append("file", data.logoData, entreprise.username + "-entreprise-offre-" + data.logoName);



                axios.all([
                    axios.post("/add-offer-entreprise/" + entreprise.id, {
                        ...data, logo: entreprise.username + "-entreprise-offre-" + data.logoName, outro: draftToHtml(data.outro)
                    }),
