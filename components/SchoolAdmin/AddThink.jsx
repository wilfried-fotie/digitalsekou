...
export default function AddThink() {
    ...
    return (
        <div>
            ...
            <center className="h1 pad">Enrichissez Votre Page</center>
...
export function Creation({ onHandleImageStateChange, onHandleTextStateChange, data, onHandleEditor, handleClick, reset }) {
    ...
    const entreprise = React.useContext(SchoolContext).data.schoolData.school
    const getPost = React.useContext(SchoolContext).data.getPost.posts
    ...
    const handleSubmit = (e) => {
        ...
        if (data.media && data.outro && data.name) {
            if (allowOnlyPicture(data.mediaName)) {
                ...
                const img = entreprise.id + "-school-post-" + post + "." + data.mediaName.split(".", -1)[1]
                setLoader(true)
                ...
                axios.all([
                    axios.post("/add-post-school/" + entreprise.id, { name: data.name, media: img, entrepriseId: entreprise.id, outro: draftToHtml(data.outro), disposition: state, proprio: "school" }),
                    ...
                    dispacth({ type: "ADDSPE", name: "getPost", pre: "posts", data: res[0].data })
...
export function Edit({ onHandleImageStateChange, onHandleTextStateChange, data, id, k, onHandleEditor, handleClick }) {
    ...
    const dispacth = React.useContext(SchoolContext).dispacth
    const entreprise = React.useContext(SchoolContext).data.schoolData.school
    const getPost = React.useContext(SchoolContext).data.getPost.posts[k]
    ...
    const handleSubmit = (e) => {
        ...
        if (data.media && data.outro && data.name) {
            if (allowOnlyPicture(data.media) || (data.mediaName && allowOnlyPicture(data.mediaName))) {
                ...
                setLoader(true)
                data.mediaName && formData1.append("file", data.mediaData, entreprise.id + "-school-post-" + id + "-" + random + "." + data.mediaName.split(".", -1)[1]);
                ...
                axios.all([
                    ...
                    dispacth({ type: "UPDATESPE", name: "getPost", pre: "posts", id: k, data: { id: id, description: draftToHtml(data.outro) || data.outro, name: data.name, media: data.mediaName && entreprise.id + "-school-post-" + id + "-" + random + "." + data.mediaName.split(".", -1)[1] || data.media, image: data.mediaName && entreprise.id + "-school-post-" + id + "-" + random + "." + data.mediaName.split(".", -1)[1] || data.media, outro: draftToHtml(data.outro) || data.outro, disposition: data.position, proprio: "entreprise" } })
...
export function Delete({ close, id, k }) {
    const dispacth = React.useContext(SchoolContext).dispacth
    const getPost = React.useContext(SchoolContext).data.getPost.posts[k]
    const data = React.useContext(SchoolContext).data
    ...
    const handleDelete = async () => {
        await axios.all([
            ...
            .then(() => {
                ...
                dispacth({ type: "DELETESPE", name: "getPost", pre: "posts",id: k })
